...
window::window(const std::string filename, std::fstream file)
    :p(filename, size)
{
    file = std::move(file);
...
void window::update_status(char *info)
{
...
void window::move_down()
{
	...
	if( x > p.text[y + y_offset].size() + 1 )
		x = p.text[y + y_offset].size() + 1;
	move(y, x);
}


/* saving and loading */
void window::load_file(const char *filename)
{
...
int count_lines(const std::fstream& file)
{
	int count = 0;
